/*
  ...
  - You are about to drop the `prod_purchase` table. If the table is not empty, all the data it contains will be lost.
  - You are about to drop the `prod_request` table. If the table is not empty, all the data it contains will be lost.
  ...
  - A unique constraint covering the columns `[id]` on the table `status_tracking` will be added. If there are existing duplicate values, this will fail.

*/
-- AlterTable
ALTER TABLE `status_tracking` ADD COLUMN `active` BIT(1) NOT NULL DEFAULT b'1';
...
-- DropTable
DROP TABLE `prod_purchase`;

-- DropTable
DROP TABLE `prod_request`;
...
CREATE TABLE `prod_colors` (
    ...
    UNIQUE INDEX `id_UNIQUE`(`id`),
    INDEX `fk_prod_colors_products1_idx`(`products_id`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `requests` (
    ...
    `tested` BIT(1) NOT NULL DEFAULT b'0',
    `active` BIT(1) NOT NULL DEFAULT b'1',
...
-- CreateIndex
CREATE UNIQUE INDEX `id_UNIQUE` ON `status_tracking`(`id`);
...
-- AddForeignKey
ALTER TABLE `colors` ADD CONSTRAINT `fk_colors_products1` FOREIGN KEY (`products_id`) REFERENCES `products`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
...
-- AddForeignKey
ALTER TABLE `prod_colors` ADD CONSTRAINT `fk_prod_colors_products1` FOREIGN KEY (`products_id`) REFERENCES `products`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
...
-- AddForeignKey
ALTER TABLE `prod_requests` ADD CONSTRAINT `fk_prod_requests_products1` FOREIGN KEY (`products_id`) REFERENCES `products`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

-- AddForeignKey
ALTER TABLE `prod_requests` ADD CONSTRAINT `fk_prod_requests_requests1` FOREIGN KEY (`requests_id`) REFERENCES `requests`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
